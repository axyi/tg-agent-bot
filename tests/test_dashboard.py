...
def llm_row(
    ...
    *,
    ...
    roles = (
        ...
        else {"system": 100, "tools": 50, "user": 10, "assistant": 0, "tool": 0}
    )
...
def test_aggregate_totals_match_the_fixture_summary():
    ...
    assert page.number("m-success_rate") / 100 == pytest.approx(summary["success_rate"], abs=1e-4)
    assert page.number("m-cost_per_success") == pytest.approx(summary["cost_per_success"], abs=5e-7)
...
def test_cache_section_reports_hit_rate_resent_and_prefix_share():
    ...
    page = Page(markup)
    summary = document_json["summary"]
    assert page.number("m-cache_hit_rate") / 100 == pytest.approx(
        summary["cache_hit_rate"], abs=1e-4
    )
    assert page.number("m-resent_share") / 100 == pytest.approx(summary["resent_share"], abs=1e-4)
    expected = (
        document_json["meta"]["prefix_tokens"]
...
def test_tool_breakdown_sums_output_tokens_and_time_over_every_run():
    ...
    assert [entry["name"] for entry in breakdown] == sorted(
        ...
        key=lambda name: (
            -next(e["output_tokens_est"] for e in breakdown if e["name"] == name),
            name,
        ),
    )
...
def test_median_run_keeps_execution_order_on_a_tie():
    ...
    tied = [
        run("S03", index, llm_rows=[llm_row(index, prompt=500, completion=50, cost=None)])
        for index in (1, 2, 3)
    ]
...
def test_timeline_rows_carry_rounds_tokens_latency_and_tool_names():
    entry = run(
        ...
        tool_rows=[
            ...
            tool_row(2, tool="fetch_url"),
            tool_row(3, tool="exec"),
            tool_row(4, turn=2, tool="history"),
        ],
    )
...
def test_context_growth_accepts_the_decoded_object_as_well_as_the_stored_text():
    rows_text = [
        llm_row(1, by_role={"system": 100, "user": 10}),
        llm_row(2, rnd=2, by_role={"system": 100, "user": 310}),
    ]
    rows_object = [
        llm_row(1, by_role={"system": 100, "user": 10}, encode=False),
...
def test_compare_section_shows_both_files_and_their_deltas():
    ...
    delta = (
        candidate["summary"]["totals"]["prompt_tokens"]
...
def test_compare_keeps_the_half_token_of_an_even_repeat_median():
    ...
    left = document(
        [
            run("S01", 1, llm_rows=[llm_row(1, prompt=1000, completion=10)]),
            run("S01", 2, llm_rows=[llm_row(2, prompt=1301, completion=10)]),
        ]
    )
    right = document(
        [
            run("S01", 1, llm_rows=[llm_row(3, prompt=900, completion=10)]),
...
def test_cli_compare_flag_adds_the_section(tmp_path):
    ...
    assert dashboard.main([str(BASELINE), "--compare", str(CANDIDATE), "--out", str(out)]) == 0
...
def test_cli_refuses_a_missing_file(tmp_path, capsys):
    assert dashboard.main([str(tmp_path / "nope.json"), "--out", str(tmp_path / "dash.html")]) == 1
