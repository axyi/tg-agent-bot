...
def conversation(conn, *, stdout=BIG_STDOUT) -> int:
    ...
    tool_turn(conn, conv, [(wire("call_2_0", "exec", {"argv": ["make"]}), exec_result(stdout))])
...
def test_hst_01_a_coerced_tool_name_gets_the_generic_stub(conn):
    ...
    tool_turn(
        ...
        [
            (
                wire("call_2_0", "unknown", {"argv": ["make"]}),
...
def test_hst_01_the_nearest_preceding_assistant_resolves_the_name(conn):
    ...
    conv = storage.get_or_create_active_conversation(conn, USER_ID)
    storage.add_user_message(conn, conv, "hello")
    tool_turn(conn, conv, [(wire("dup", "exec", {"argv": ["make"]}), exec_result("first"))])
    tool_turn(
        conn,
        conv,
        [
            (
                wire("dup", "fetch", {"url": "https://wttr.in/x"}),
    ...
    stubs = [json.loads(message["content"]) for message in tool_messages(assemble(conn, conv))]
...
def test_hst_01_the_stub_head_never_ends_inside_a_secret(conn, monkeypatch):
    ...
    tool_turn(
        conn, conv, [(wire("call_2_0", "exec", {"argv": ["cat", "key"]}), exec_result(stdout))]
    )
    storage.add_user_message(conn, conv, "again")
...
def test_hst_02_the_latest_load_of_each_skill_is_verbatim(conn):
    ...
    tool_turn(
        ...
        [
            (
                wire("call_2_0", "load_skill", {"name": "weather"}),
    ...
    tool_turn(
        ...
        [
            (
                wire("call_3_0", "load_skill", {"name": "weather"}),
    ...
    tool_turn(
        ...
        [
            (
                wire("call_4_0", "load_skill", {"name": "hosts"}),
    ...
    assert json.loads(contents[0]) == {"stub": True, "tool": "load_skill", "name": "weather"}
...
def test_hst_03_user_and_assistant_messages_are_never_stubbed(conn):
    ...
    assert [message["role"] for message in plain] == ["user", "assistant", "assistant", "user"]
...
def test_the_example_payload_shrinks_by_an_order_of_magnitude(conn):
    ...
    conv = conversation(conn)
    verbatim = len(json.dumps(assemble(conn, conv, stub_tool_results=False), ensure_ascii=False))
