...
a guard by violating it. **62 ids, seventeen of them negative**, every one
...
| `T-V190-STO-02` | a seeded v5 database (built by executing v5's `_SCHEMA` + `_MIGRATION_4_TO_5` and inserting rows into `llm_calls`, `spans`, `tool_calls` **and** `messages`) migrates to 6: all four tables preserved by **count and content**, `PRAGMA foreign_key_check` returns no rows, `idx_llm_calls_conv` exists again, the `rerank` purpose now insertable, `documents`/`chunks` present, version 6; **`PRAGMA foreign_keys` reads 1 after the migration returns**; a fresh database is 6; version 7 refused (the EC-03 amendment) |
| `T-V190-STO-09` | negative: a failure injected mid-migration (a monkeypatched `conn.execute` raising on the `INSERT INTO llm_calls_v6 … SELECT` step) propagates out of `_migrate_5_to_6`, and afterwards the schema-5 database is **intact and usable** — `schema_version` still 5, `llm_calls` at its seeded count and content, no `llm_calls_v6` and no `documents`/`chunks` left behind — with `PRAGMA foreign_keys` back at **1** and `PRAGMA foreign_key_check` empty; a `BaseException` (`KeyboardInterrupt`) injected at the same point rolls back the same way |
...
| `T-V190-RET-08` | `Searcher.search` returns ≤ `rag_top_k` hydrated passages, `documents_present` false with no rows, `calls` recorded; `RAG_RERANK=off` makes no LLM call **and yields `rerank_attempted=False`, `rerank_succeeded=False`, `rerank_failure=None`** — as does a single candidate with rerank on; a clean rerank yields `True`/`True`/`None`; the reranker receives `list[Passage]` already carrying filename and text (never bare ids), and `chunks_by_ids` is called exactly once per `search` |
...
| `T-V190-RET-11` | negative: the rerank flags cannot claim a rerank that did not apply — a **malformed-JSON** reply yields `rerank_attempted=True`, `rerank_succeeded=False` and a **non-empty** `rerank_failure`, with the passages in hydrated RRF order; the same three values result when `_record_llm_call` raises **after** a well-formed reply was parsed (the bookkeeping is inside the guard, so `rerank_succeeded` is never set), proving that an `llm_calls` row is not evidence of a successful rerank; the flags never appear in `execute_tool`'s envelope text or its `passages` count |
...
| `T-V190-TOOL-06` | `attach_sources`, structural by canonical rendering: no source line → `Sources: a.pdf (page 2), b.md`; a line equal to the rendering of a **subset** of the returned pairs → untouched, under either the `Source:` or the `Sources:` prefix; **an incidental prose mention ("I read policy.pdf") still gets the block**; max 5; pages grouped as `(pages 2, 5)` in first-seen order |
...
| `T-V190-TOOL-08` | negative: no filename is ever parsed by delimiter — with the returned pair `("a, b (page 9).pdf", 9)` the line `Sources: a, b (page 9).pdf (page 9)` **survives** on whole-string equality, while `Sources: a, b (page 9).pdf` (page dropped), `Sources: a` and a line adding `invented.pdf` are each **stripped** with one warning and the canonical block appended; with five returned pairs the ≤ 31 subset renderings are enumerated once and a line equal to any of them survives in its own order |
...
| `T-V190-CMD-03` | `download_file` streams through `httpx.MockTransport`, stops at the cap, maps ≠ 200 and **non-timeout** transport errors to `TelegramError` without the token in the message (a timeout is `T-V190-CMD-10`'s subtype, never a plain `TelegramError`) |
| `T-V190-CMD-04` | progress: the exact four stage strings in order on the fake's `edited` — and **no other** progress string, in particular none for classify or store; the `progress` callable `index_document` receives emits only `📄 extracted`, `📄 chunked` and `📄 embedding: i/n`, while `📄 received` and both endings come from the handler; one deletion on success, one confirmation message with chunks (and pages for a PDF); on failure the last edit is the error line and no deletion |
...
| `T-V190-CMD-07` | the indexing budget, measured as `monotonic() - started_at`: a fake clock jumping past 300 s between stages → the timed-out line, no rows; **`IndexBudgetExceeded` raised inside the PDF page loop still gives row 10c's exact message, never row 2's** |
...
| `T-V190-CMD-09` | negative: the budget counts the handler's pre-index work — a fake clock advanced by **290 s before `index_document` is entered** (across `getFile` and the download) and by a further **20 s inside** it gives ERR-01 row 10c's exact line and zero rows, which a clock started at `index_document`'s own entry could not produce; `started_at` is the first value the handler reads, and `index_document` refuses to be called without it (no default) |
| `T-V190-CMD-10` | negative: through the **real `download_file`** with an `httpx.MockTransport` raising `httpx.ReadTimeout` — a `TelegramDownloadTimeout` is raised (a `TelegramError` subclass, `__cause__` set, no token in the message) and the handler answers with ERR-01 row 10a's exact line, never row 11's; a `httpx.ConnectError` through the same boundary still gives a plain `TelegramError` and row 11 |
| `T-V190-ERR-01` | one test per ERR-01 row 1–7, 10a–c, 11, 13–15: the exact user string, the log line prefix, and `documents`/`chunks`/`vec_chunks` counts unchanged; **row 13 is asserted on both of its paths** — the handler's pre-`COUNT` (a plain reply, no status message and no `getFile` call) and DOC-04's **transactional recheck** (a 20th document inserted behind the pre-check, so the in-flight index reaches the insert and is refused with the same string, leaving zero new rows) |
...
| `T-V190-EVAL-02` | `rag_eval` end-to-end with `FakeEmbedder`/`FakeLLM` on the real corpus: renders DOCX and PDF in memory, indexes for user −1, prints the table, exit 0/1 by the floor, **exit 2 when one answerable item's `SearchResult` comes back with `rerank_succeeded=False`** (a scripted malformed reply), the exit message naming that item and its `rerank_failure`, 2 when the embedder raises or the chat client cannot be built; `vector` and `hybrid` call `chunks_by_ids(user_id=-1)` **once each** and score in ranked-id order, not row order |
...
| `T-V190-EVAL-04` | offline, before any live run: every answerable item's `expected_evidence` occurs in the extracted text of its `expected_source` (case-insensitive, whitespace-normalised) — a corpus defect fails here, never at the live gate; **the override and the exit-2 path**: with `RAG_RERANK=off`, `RAG_TOP_K=3` set the run still reranks and still cuts at 5 (echoed in the printed header); a chat client that cannot be constructed or called, and **any answerable item lacking `rerank_attempted is True and rerank_succeeded is True`**, each exit 2 — the gate reads the flags, never an `llm_calls` count or a log line; no path returns 0 while the rerank mode reads `n/a` |
...
Scenario: E9 — the reranker fails and the answer still completes
  ...
  And the SearchResult reports rerank_attempted true, rerank_succeeded false and a non-empty rerank_failure
  And with RAG_RERANK off the same search reports attempted false, succeeded false, failure None
...
Scenario: E11 — the structural Sources line is appended
  ...
  And a source line whose whole text equals the canonical rendering of a subset of the returned pairs survives, under either the "Source:" or the "Sources:" prefix
  And one also naming "invented.pdf" matches no rendering and is removed
  And a returned filename containing commas and parentheses — "a, b (page 9).pdf" — is never split on them
