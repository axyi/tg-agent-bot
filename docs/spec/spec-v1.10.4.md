...
Status: draft — cross-review pending (Appendix C is a placeholder).
...
sit in `stash@{0}` (`v1103-T6 mutations part: uncommitted at
...
`${TMPDIR:-/tmp}/v1104-gate8-${tested_tree}.log`; **the stash is read
with `git stash show -p stash@{0}` and tested with `… | git apply
--check -` only** — never `pop`, `apply` or `drop` (NG-03). **The
...
may invoke gates 5 and 7 once each, never gate 8. No `bench.py`; no LM
...
amendment list, the structural T0 inventory, and the rule that a missed
pin is an amendment, never a cycle.** Write §6's tests, watch them fail
...
**The T0 structural inventory is the primary tool; the literal list is
its check.** T0 runs (commands only, before any live call):
...
the spec/report names). **Every hit is either an EC-02 row (by number)
or a one-line "unaffected because …"** in the report's T0 hit table
(RPT-01). **A pin the inventory missed, tripped after T0, is a
...
`v1104-T1.md`, `-T2` and `-T5`, plus `v1104-T3.md` iff T3 delegates a
source-writing fix**; T0 and T4 are *commands only* (T4's one yaml
number *a single edit under every threshold*); T5's evidence commit is
*artefacts only*. §10.1's `delegate` column defaults to **yes** for any
...
REV-02); `--no-verify` is never used and the report attests it.
...
paths.** No `v1104-*` entry exists because this release adds no
...
strings are the stash's, verbatim; on the fallback they are re-authored
from this table and `report-v1.10.3.md:566-572`. **Procedure (T2,
repository root, after T1's commit)**:
...
§1); T1 does not touch these two files. **The stash's three test hunks
...
2b). If `git apply` exits non-zero (ERR-01 row 1), T2 re-authors from
this section; either way the result is verified against this table. **Each entry is
...
(exit 0) **and** the named killer is red under `pytest -k` — command,
import exit and failing assertion recorded (RPT-01). `len(MUTATIONS)` becomes **144**. `T-V1104-MUT-01`,
...
calibration comment in `:574-579`'s shape (GATE-02); (c) in the `mutation-all` comment block (`:590-707`) the
...
becomes `MUTATIONS\)`\s*is now (\d+)` (provenance lives in prose the
...
from the first commit; each later task appends its section and bullet. `T-V1104-RPT-01`, `T-V1104-PIN-05`,
...
|---|---|---|---|---|
| 1 | T2, `git apply` | the two-path apply exits non-zero | the fallback (§3), verified against the table; recorded | no cycle |
...
| 3 | T2, isolation | an entry not killed by its named killer alone | `REQ-V1103-ERR-01` row 7: disclose; rewrite syntax-preserving | a repair cycle |
...
| 7 | T4, calibration | `2 × wall + 70 s` rounded up to 10 s differs from 110 | `mutation-v1103.timeout_seconds` set to the computed value | recorded |
...
restorable → Stage B at T5 (ERR-01 row 13). **The reuse record** states
...
commands only: (1) `uv run --locked python devtools/mutation_check.py
--select v1103-` **once**, wall measured; `timeout_seconds` = wall × 2 +
70 s, rounded up to 10 s, replaces the placeholder 110 with a dated
comment in `:574-579`'s shape (result expected 5/5, recorded, no
verdict) — *a single edit under every threshold*; `[[VERIFY: v1102's
...
record** (command, exit, or the fallback and its reason); **the
per-entry isolation proofs** (§3 row, import-proof exit, the `pytest
-k` command, its result, the failing assertion); **the T0 inventory
...
keeps its intent; no test deleted; (3) every T0 inventory hit is an
EC-02 row or a one-line reason; every post-T0 amendment is disclosed; (4) the isolation proofs
...
hold; no source module changes (`T-V1104-DOC-05`); every `T-V1103-*`
...
`mutation-v1103` `N/A` when never landed), `gitleaks` exit 0, the
...
commits, REV-02); tests before the code they cover, in the same task.

| T | task | acceptance |
|---|---|---|
| **T0** | Preflight (*commands only*): hooks, `doctor`, **test count re-measured** (2285 at authoring; the floor), `len(MUTATIONS)` 139, last prompt 227, last usage row 138 (both written by the authoring commit that lands this spec; at `f3ce1a5` they are 226 / 137), `<base>` `f3ce1a5`, the spec's `sha256`; `git stash list` shows `stash@{0}` and the two-path `git apply --check` exits 0 (or the T2 fallback is announced now); EC-02's inventory; the seven Stage 0 checks; gates 1–5 and 7; `docs/prompts/228-go-spec-v1.10.4.md`; the report skeleton (RPT-01) | every item recorded; the hit table; the fallback record; no key value anywhere; `git diff --exit-code` clean after check 7 |
...
| **T2** | MUT-01, MUT-02: the two-path `git apply` (or the fallback), the "is now" sentence, each entry re-verified in isolation, EC-02 row 2b. `T-V1104-MUT-01…05`, `T-V1104-ERR-01`; gates 1–4 | 5/5 killed in isolation with the proofs recorded; `len(MUTATIONS)` 144; the `git apply` exit recorded; `stash@{0}` still listed; `doctor` green |
...
| **T4** | GATE-01's T4 sequence and GATE-02's steps (1)–(2), verbatim (*commands only*; the one yaml number *a single edit under every threshold*); commit between gate 6 and the live sequence; gate 8 last and once | the calibration line; `mutation-all` 144/144 with its wall; the write-tree and `HEAD^{tree}` equal; gates 1–7 green; the attempt-log rows; `gate8_exit` 0 with the floors and judge mean met, the per-case table, every `CASE`/`TOOLS`/`FAIL` line, "on this run"; exit 1 is Stage B′; a missing capture is ERR-01 row 11 |
| **T5** | REV-02 verbatim — one prompt (233), two commits: the first (brief `v1104-T5.md` for the test files; the literal and `uv lock` *a single edit under every threshold*) lands VER-01, EC-02 rows 15–16, RPT-03's T5 part, the provisional artefacts; gates 1–7, the identity check, the collection check, `replay`, `E1`–`E7`; the second (*artefacts only*) is the evidence commit; `E8`, `lint-docs`, `gitleaks-tree`; the tag; **no push**. `T-V1104-VER-01`, `-VER-02`, `T-V1104-DOC-02`, `-DOC-04` | the four T5 tests red before, green after; `T-V1104-VER-02`; `git diff <tested_tree> HEAD -- config/quality_gates.yaml` empty; the reuse facts, gate 8 executed once; the count ≥ floor + 21; the evidence commit's `git show --stat` names exactly the four paths; `E8` green before the tag; no push in the command record |
...
| **T4** | §7, §4 (rows 7–12), §9 (Stage B′, B″) | `config/quality_gates.yaml` — the `mutation-v1103` block only; `docs/spec/spec-v1.10.2.md:449-462`; `docs/spec/spec-v1.10.3.md:730-778` | no — *commands only*; the one yaml number is *a single edit under every threshold* |
| **T5** | §8, §9 (REV-02), §7 (the identity check), §4 (rows 12–13), §1 (rows 15–16, the floor) | first commit: `pyproject.toml` (`project.version` only); `README.md:594-600`, `:913-916`; `AGENTS.md:159-172`; `tests/test_v195_version.py`, `tests/test_v194_version.py:25-34`, `tests/test_v190_agents.py:134-152`; `tests/test_v1104_version.py`, `tests/test_v1104_docs.py`; second commit: this run's artefacts | **yes** for the test files — brief `v1104-T5.md`; **no** for the literal and `uv lock` (*a single edit under every threshold*) and the evidence commit (*artefacts only*) |
...
|---|---|
| `REQ-V1104-EC-01` | `T-V1104-VER-02`; the gate tables and command record; `T-V1104-DOC-03` |
| `REQ-V1104-EC-02` | the T0 count and T5 check; the T0 hit table; post-T0 amendments; `T-V1104-PIN-01`, `T-V1104-PIN-06` |
| `REQ-V1104-EC-03` | §10.1; the briefs `v1104-T1.md`, `-T2`, `-T5`, `-T3` iff a fix; `T-V1104-RPT-01` |
| `REQ-V1104-EC-04` | T0 check 1's output; `db_empty=True`; the `describe()` pairs; `replay --range`; `gitleaks-tree`; `E7` |
| `REQ-V1104-MUT-01` | `T-V1104-MUT-01`, `T-V1104-MUT-02`, `T-V1104-MUT-05`; the stash record and the per-entry proofs; `E1` |
...
| `REQ-V1104-ERR-01` | `T-V1104-ERR-01` (row 2); `T-V1103-LINT-02` and `T-V1104-PIN-07` (row 5); the task records; `E5` |
...
| `REQ-V1104-GATE-02` | `T-V1104-PIN-05`, `T-V1104-MUT-03`; the calibration and write-tree records; `E1` |
...
| `REQ-V1104-REV-03` | the stage named in the report; the negative proofs; the reused gate-8 record; `T-V1104-RPT-01` on the stop-route report |
...
| 8 | the stash's untracked parent `043842f` collides on `pop`/`apply` (facts §1); its three test hunks carry the frozen shapes | `MUT-01` (the two-path apply; test hunks not applied); `NG-03`; the post-push `stash drop` (REV-02) |
