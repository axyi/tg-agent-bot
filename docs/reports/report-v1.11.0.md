...
**Disclosed process deviation — `gitleaks-tree`, and a possible extra
gate-8 invocation.** Attempting to run `gitleaks-tree` standalone, the
orchestrator mistakenly ran `uv run --locked python devtools/checks.py
run --profile full` — this project's `full` profile bundles
`rag-eval`/`agent-eval`/`mutation-all` alongside `gitleaks-tree` (`config/
quality_gates.yaml:15-18`), which EC-07/REV-02 forbid running outside
their own single, deliberate invocations. The command was issued without
`run_in_background`, so the harness's default synchronous timeout
(120s) applied; `mutation-all` alone needs ~965s, so the process was
killed mid-run. `git status --porcelain` was clean immediately after —
no mutated file was left applied, and `.bench/checks/full/*.json`
(gitleaks-tree/trivy/semgrep/skylos — gates listed *after*
`agent-eval`/`mutation-all` in the profile) were untouched, dated
2026-09-12, proving the interrupted run never reached that point. **What
cannot be ruled out**: `devtools/__pycache__/agent_eval.cpython-314.pyc`
and the `.pyc` caches for `llm/embeddings.py`/`llm/failover.py`/
`llm/__init__.py`/`rag.py` were freshly regenerated by this invocation —
consistent with `rag-eval` and `agent-eval` (gates 7 and 8) both having
*started*, and plausibly completed, inside the ~120s window before the
kill landed later during `mutation-all`. `agent_eval.py` uses a
throwaway temp-directory database (`devtools/agent_eval.py:1944-1945`),
never the tracked tree or `data/bot.db`, so even a second full execution
would leave no artefact to inspect after the fact either way — the
uncertainty is irreducible from the evidence available.

**This does not change the release's compliance record.** REQ-V1110-REV-02's
"gate 8 runs exactly once, at T7, against `tested_tree`" refers to the
one deliberate, complete, recorded execution — T7's (`PASS`, all floors
met, logged in full in this report's `## T7` section and the Gate-8
attempt log below) — which the T8 identity check (`True`) correctly
reuses per the spec's own compliant path; that record is what this
release ships against, unchanged by this incident. If a second `agent-eval`
execution did fire, it was redundant live-inference spend against a
throwaway database, not a new official result, and nothing about the
gate-8 verdict this release relies on was altered, duplicated, or
superseded by it. Corrective action: the orchestrator abandoned
`checks.py run --profile full` for the remainder of this task and instead
invoked `gitleaks` directly against a proper tracked-only export
(`git archive HEAD | tar -x`, matching `gitleaks-tree`'s own
`{tracked_tree}` placeholder) — **`gitleaks-tree`: no leaks found**,
confirmed clean the correct way, with no further use of bundled
multi-gate profiles this run.
