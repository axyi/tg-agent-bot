...
| `docs/spec/spec-v1.6.0.md` + implementation | **STOPPED at T15 of 19, operator decision (2026-09-06) — no tag, no baseline.** T0–T14 fully landed and gated: the observability layer (`tracing.py` spans, storage schema 4, `agent.py` wiring), the live read-only dashboard (`dashboard_render.py`/`dashboard_server.py`, on by default, loopback-only, plus `/status`'s new line), three tool-quality fixes (truncated-summary retry, closed outcome vocabulary, repeat-call refusal), six new benchmark scenarios S13–S18 with a `tool_calls_max` ceiling, `bench_schema` 2, the version bump (`pyproject.toml` → `1.6.0`), the `mutation-v160` gate (11 entries, 83 total), and a clean-context code review (five findings, all fixed). T15 resolved the live preflight (REQ-V160-PRE-04's `max_tokens=16` corrected in place, disclosed, to `cfg.llm_max_tokens`) but then found `smoke-v160`'s `S13` scenario deterministically exceeds its own new `tool_calls_max(4)` check (5/5 across repeats, always the correct answer) — REQ-V160-TQ-06's check working as designed, not a defect, and not loosened (`advisor()` consulted, same `QUALITY_GATE_SLACK` precedent as v1.6.0's own T10). T16 requires 3/3 on every one of S13–S18, so the baseline cannot be recorded with this model at this ceiling; the operator chose to accept the stop rather than authorise a scoped correction. **T16 (baseline), T17 (report finalisation) and T18 (final acceptance + the `v1.6.0` tag) were never executed.** `pyproject.toml`'s `project.version` already reads `"1.6.0"` (landed at T12, before the stop) — REQ-V160-VER-04's tag-creation step never ran, so **no `v1.6.0` git tag exists**; the version string and the git history disagree on whether "1.6.0" shipped until a future run either completes T16–T18 on this tree or corrects the version string back down. 1015 tests as of T14 (843 at T0). See `docs/spec/spec-v1.6.0.md`; `docs/reports/report-v1.6.0.md` is the full record, closed at T15 |
| `docs/spec/spec-v1.7.0.md` + implementation | **in progress** — a minor release: a per-purpose reasoning policy (`LLM_REASONING_POLICY`/`LLM_REASONING_ON_PURPOSES`, resolved through frozen `ReasoningRequest`/`ReasoningMechanism` types in `llm/base.py`), a summary wall-clock budget shared across attempt 1 and the retry/repair, and `devtools/bench.py`'s comparability/gate rules extended for the new treatment (`meta.reasoning`, the S13–S18 executable 3/3 gate, `--tag` path-safety). Stage A (T2–T3) probed five LM Studio reasoning-off mechanisms live against the GPU-hosted server: only the `summary` purpose's assistant-prefill mechanism (candidate c) proved both honored and shippable; `tool-round`/`final` have no known working off-switch on this hardware, so RSN-07's summary-only fallback bounds what this release can ship for those two tags. T0–T8 landed (schema migration 4→5, provider wiring, the budget, bench.py's BEN-02…-08/CAR-01 gates); T9–T14 (mutation entries, review, the live stage-C candidate benchmark, the conditional selection commit, final report and tag) remain. 1133 tests as of T8 (1122 at T7's close, 1004 at v1.6.0 T12). See `docs/spec/spec-v1.7.0.md`; `docs/reports/report-v1.7.0.md` is the running record |
...
## v1.5 (complete) — spec: `docs/spec/spec-v1.5.md`, report: `docs/reports/report-v1.5.md`
