...
Gates 1–4 are unconditional and offline (gate 3, `pytest`, is 1593
tests as of spec-v1.9.1 T2). Gate 5 needs the live environment
...
the test suite once per mutation) — 108 entries as of v1.9.1 T3 (up
...
cleanup, the upload size precheck, and the sources-fallback rendering, T1
added two more `v191-*` entries covering the rerank `response_format`
contract and `LLM_RERANK_MODEL` routing, and T3 added one more
(`v191-rerank-retry-dropped`, covering the retryable-429 retry loop); see
`docs/reports/report-v1.9.0.md` and `docs/reports/report-v1.9.1.md`);
